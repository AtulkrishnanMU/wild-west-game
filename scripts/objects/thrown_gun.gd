...
const GUN_SPEED = 700.0  # Slightly faster than bat
const GUN_RETURN_SPEED = 900.0
const GUN_TRAVEL_DISTANCE = 350.0
const SPIN_SPEED = 25.0  # Faster rotations per second for gun

@onready var sprite: Sprite2D = $Sprite2D

@export var direction: Vector2 = Vector2.RIGHT
@export var start_position: Vector2
@export var thrower: Node2D
var rotation_angle: float = 0.0
@export var damage: int = 30  # Slightly more damage than bat
var spin_sound_player: AudioStreamPlayer2D = null
var bounce_count: int = 0
var max_bounces: int = 2  # Fewer bounces for gun (heavier)
var can_bounce: bool = true
var bounce_cooldown: float = 0.15  # Shorter cooldown for gun
...
var is_returning: bool = false  # Track if gun is in auto-return mode
var return_timeout: float = 2.5  # Shorter timeout for gun (faster weapon)

# Fallback auto-return variables
var fallback_timeout: float = 6.0  # Shorter total timeout for gun
var stuck_detection_time: float = 1.5  # Shorter stuck detection
var last_position: Vector2 = Vector2.ZERO
var stuck_timer: float = 0.0

# Speed variation variables
var current_speed: float = GUN_SPEED  # Current movement speed
var min_flight_speed: float = GUN_SPEED * 0.4  # 40% of base speed at max distance
var speed_transition_distance: float = GUN_TRAVEL_DISTANCE * 0.7  # Start slowing at 70% distance
...
var total_flight_time: float = 0.0    # Total time gun has been in flight
...
func _physics_process(delta: float) -> void:
	...
	if thrower and is_instance_valid(thrower) and thrower.has_method("is_dead") and thrower.is_dead:
		...
		direction.y += 400.0 * delta  # Stronger gravity for gun (heavier)
		direction.y = min(direction.y, 900.0)  # Higher terminal velocity
		# Move with gravity
	...
	if current_distance >= GUN_TRAVEL_DISTANCE:
...
func _on_body_entered(body: Node) -> void:
	...
	if body is RigidBody2D:
		# Apply stronger impulse to RigidBody2D (gun is heavier)
		var impulse = direction * 400.0  # Stronger impulse than bat
	...
	if body.is_in_group("enemies") and ("is_active" in body) and not body.is_active:
		...
		if notifier and notifier.is_on_screen():
			...
			CharacterUtils.apply_knockback(body, knockback_dir, 350.0, 0.2)  # Stronger knockback
	...
	if body.has_method("take_damage"):
		body.take_damage(damage)
		# Apply stronger knockback for gun
		var knockback_dir = sign(body.global_position.x - global_position.x)
		CharacterUtils.apply_knockback(body, knockback_dir, 350.0, 0.2)
	...
	if owner and owner != thrower:
		...
		if owner.is_in_group("enemies") and ("is_active" in owner) and not owner.is_active:
			...
			if notifier and notifier.is_on_screen():
				...
				CharacterUtils.apply_knockback(owner, knockback_dir, 350.0, 0.2)
				# Create impact effect
...
func _setup_spin_sound():
	...
	spin_sound_player.pitch_scale = 1.4  # Higher pitch for gun spinning
...
func _create_metal_sparks(pos: Vector2, normal: Vector2) -> void:
	MetalSparkUtils.create_metal_sparks(pos, normal, "gun")
...
func _update_spin_speed(delta: float) -> void:
	# 1. INITIAL THROW - Accelerate into spin (first 0.15 seconds for gun)
	if total_flight_time < 0.15:
		spin_acceleration_time += delta
		# Quick ramp-up from 0 to peak speed
		var acceleration_progress = spin_acceleration_time / 0.15
		current_spin_speed = acceleration_progress * 1.3  # Higher peak for gun
	...
	# 2. MID-FLIGHT - Maintain fast spin with slight decay
		...
		var air_resistance_factor = 0.97  # Slightly less decay than bat (gun is more aerodynamic)
		current_spin_speed = current_spin_speed * air_resistance_factor
		current_spin_speed = max(current_spin_speed, 0.85)  # Don't go below 85% in mid-flight

func _apply_bounce_spin_loss() -> void:
	# Bounce impact reduces spin speed by 15-25% (gun loses less spin than bat)
	var spin_loss_factor = randf_range(0.75, 0.85)  # Keep 75-85% of current spin
	current_spin_speed = current_spin_speed * spin_loss_factor
	current_spin_speed = max(current_spin_speed, 0.3)  # Never go below 30% spin
...
func _update_flight_speed() -> void:
	...
	if is_returning:
		...
		var return_progress = 1.0 - (distance_to_player / GUN_TRAVEL_DISTANCE)
		return_progress = clamp(return_progress, 0.0, 1.0)  # Prevent negative values
		current_speed = lerp(min_flight_speed, GUN_RETURN_SPEED, return_progress)
	else:
		...
		if distance_traveled <= speed_transition_distance:
			...
			current_speed = GUN_SPEED
		else:
			# Gradual slowdown from 70% to 100% distance
			var slowdown_progress = (distance_traveled - speed_transition_distance) / (GUN_TRAVEL_DISTANCE - speed_transition_distance)
			current_speed = lerp(GUN_SPEED, min_flight_speed, slowdown_progress)
	...
	if ray_result:
		if ray_result.collider and (ray_result.collider is TileMap or ray_result.collider.is_in_group("colliders")):
			if can_bounce:
				...
				# After 2 bounces, always return (gun is heavier)
				if bounce_count >= 2:
					is_returning = true
					_handle_auto_return(delta)
					return
...
func _pickup_gun():
	# Gun picked up by player
	is_returning = false  # Reset return state
	if thrower and is_instance_valid(thrower):
		if thrower.has_method("_on_gun_returned"):
			thrower._on_gun_returned()
	queue_free()
...
func _check_fallback_conditions():
	...
	if total_flight_time >= fallback_timeout:
		# Fallback: Gun timeout exceeded, forcing return
	...
	if last_position != Vector2.ZERO:
		...
		if movement_distance < 1.0:  # Barely moved
			...
			if stuck_timer >= stuck_detection_time:
				# Fallback: Gun appears stuck, forcing return
...
func _force_fallback_return():
	...
	if return_sound:
		AudioUtils.play_positioned_sound(return_sound, global_position, 1.3, 1.6)
	
	# Fallback activated: Gun returning to player (ignoring collisions)

# DEBUG: Helper functions to detect invalid gun states
